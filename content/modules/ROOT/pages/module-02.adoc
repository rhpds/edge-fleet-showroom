...
NOTE: Unless you are assigned as 'user1' in this Lab, the user ID shown in the following screenshots will contain a different user ID other than your own.
...
Note that a QR code is available to enable further enrollment.   

=== QR Enrollment via Mobile Phone
If you have a Mobile Device, continue with the steps indicated below.  If you do not have
...
IMPORTANT: Unless you have been assigned 'user1' do not use 'user1' as your Username (as shown in the screenshot below). Please utilize the name that you have been assigned: {user}
...
Since you have enrolled via a Mobile Device, you can skip the next section.

. Click *Device Details* in the right-hand Lab Manual Contents directory

image::m2-devicedetails.png[link=self, window=blank, width=50%]

=== QR Code Enrollment via Web Browser

Ken: Enter Steps (verify with Patrick that Lab Terminals can use PRINTSCR Key)
...
NOTE: The search filter in step 5 above *is* case-sensitive
...
. *Fleet name: None* - when we enrolled the device, we did not join it to a Fleet (we will do so in a moment)
. Click *Terminal*
...
. Note that we receive an, *"Application is not available"* error message (this is expected at this time because our 'base' device image does not contain
the custom point of sale application).  
...
Keep the Application Browser Tab open and return to the Routes Tab in the OpenShift UI
...
Once the device is reporting, *'Online'* and *'Up-to-date'*:
...
The '*pos-v1-prod:1.0.0*' container listed is our point of sale application which is accessed via the front of store pos kiosk's.
...
how Edge Manager automatically swapped this image to one containing the POS App (after joining a Fleet that has this desired state configuration). You may be wondering:  "Why not just 
join the Fleet as part of onboarding?" and the answer is that you certainly could have done that but then you may not have learned as much about how Edge Manager functions.  Additionally, you 
may have wondered, "Why not just bake the app(s) into the 'base' image?" and that is also certainly an option. The great thing about Edge Manager is that it gives users the optionality to configure
things as they see fit, in alignment with any of their current build standards that might exist. 

Regardless of the approach taken, it should hopefully be more evident to you that Edge Manager can play a significant role in preventing configuration drift for edge devices.  

=== Up Next

You are well on your way to opening your new store for business; however, there is one small challenge. You may have noticed that our POS app is not the most useful (it doesn't actually do anything
other than present a static user interface with no interoperability).  In our final module, you will learn how to create a new fleet from scratch and specify a new point of sale application as part
of the fleet device template.  You will also explore how to monitor devices along with how to utilize the Edge Manager command line interface.