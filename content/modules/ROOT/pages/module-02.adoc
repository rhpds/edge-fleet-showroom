...
NOTE: Your edge device has powered on and established a secure mTLS connection with the Edge Manager API Service. As part of the enrollment process,
...
a Mobile Device, skip this section by click https://showroom-showroom-rhfnr-{user}.apps.cluster-rhfnr.dynamic.redhatworkshops.io/content/modules/module-02.html#_qr_code_enrollment_via_web_browser[here] to go to the *QR Code Enrollment via Web Browser* section (you can also select this section in the *Contents*
...
IMPORTANT: Since you just enrolled via a Mobile Device, you do not need to follow the instructions in the next section, "QR Code Enrollment via Web Browser".  Click https://showroom-showroom-rhfnr-{user}.apps.cluster-rhfnr.dynamic.redhatworkshops.io/content/modules/module-02.html#_device_details[here] to skip to the next section.
...
. Press Lab Device *PRT SC* (Print Screen) key 
...
. Enter the *hometown name you chose* for your device (case sensitive) to filter the device list
